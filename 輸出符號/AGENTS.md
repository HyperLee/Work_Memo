...
This directory contains the .NET 10 console project `輸出符號`.

- `輸出符號/輸出符號.csproj` is the SDK-style executable project targeting `net10.0`.
- `輸出符號/Program.cs` is the preserved console entry point and teaching example.
- `輸出符號.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 輸出符號` configuration and its matching build task.
