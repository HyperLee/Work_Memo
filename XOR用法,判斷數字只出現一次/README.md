...
原始需求與題目說明保留在 `XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次.csproj
dotnet build XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次.csproj --nologo
dotnet run --project XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次.csproj --no-build --nologo
```
...
```text
.
├── XOR用法,判斷數字只出現一次/XOR用法,判斷數字只出現一次/
│   ├── Program.cs
│   └── XOR用法,判斷數字只出現一次.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
