...
This directory contains the .NET 10 console project `list_reverse`.

- `list_reverse/list_reverse.csproj` is the SDK-style executable project targeting `net10.0`.
- `list_reverse/Program.cs` is the preserved console entry point and teaching example.
- `list_reverse.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug list_reverse` configuration and its matching build task.
