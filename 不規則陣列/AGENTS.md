...
This directory contains the .NET 10 console project `不規則陣列`.

- `不規則陣列/不規則陣列.csproj` is the SDK-style executable project targeting `net10.0`.
- `不規則陣列/Program.cs` is the preserved console entry point and teaching example.
- `不規則陣列.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 不規則陣列` configuration and its matching build task.
