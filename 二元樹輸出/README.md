...
原始題目、需求與參考連結保留於 二元樹輸出/二元樹輸出/Program.cs 的 XML 註解。
...
dotnet restore 二元樹輸出/二元樹輸出/二元樹輸出.csproj
dotnet build 二元樹輸出/二元樹輸出/二元樹輸出.csproj --nologo
dotnet run --project 二元樹輸出/二元樹輸出/二元樹輸出.csproj --no-build --nologo

本機只有 .NET 10 runtime；net8.0 可執行時使用：
DOTNET_ROLL_FORWARD=Major dotnet run --project 二元樹輸出/二元樹輸出/二元樹輸出.csproj --no-build --nologo
...
```text
[翻轉完整二元樹]
Expected: inorder=9,7,6,4,3,2,1;preorder=4,7,9,6,2,3,1;postorder=9,6,7,3,1,2,4
Actual: inorder=9,7,6,4,3,2,1;preorder=4,7,9,6,2,3,1;postorder=9,6,7,3,1,2,4
PASS-FAIL: PASS
[翻轉空樹]
Expected: null
Actual: null
