...
原始需求與題目說明保留在 `Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/Dictionary存陣列資料輸出.csproj
dotnet build Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/Dictionary存陣列資料輸出.csproj --nologo
dotnet run --project Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/Dictionary存陣列資料輸出.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/Dictionary存陣列資料輸出.csproj --no-build --nologo
```
...
Actual: [ate,eat,tea]|[bat]|[nat,tan]
PASS-FAIL: PASS
...
```text
.
├── Dictionary存陣列資料輸出/Dictionary存陣列資料輸出/
│   ├── Program.cs
│   └── Dictionary存陣列資料輸出.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
