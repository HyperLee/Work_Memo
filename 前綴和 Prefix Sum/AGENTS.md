...
This directory contains the .NET 10 console project `前綴和 Prefix Sum`.

- `前綴和 Prefix Sum/前綴和 Prefix Sum.csproj` is the SDK-style executable project targeting `net10.0`.
- `前綴和 Prefix Sum/Program.cs` is the preserved console entry point and teaching example.
- `前綴和 Prefix Sum.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 前綴和 Prefix Sum` configuration and its matching build task.
