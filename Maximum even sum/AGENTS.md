...
This directory contains the .NET 10 console project `Maximum even sum`.

- `Maximum even sum/Maximum even sum.csproj` is the SDK-style executable project targeting `net10.0`.
- `Maximum even sum/Program.cs` is the preserved console entry point and teaching example.
- `Maximum even sum.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug Maximum even sum` configuration and its matching build task.
