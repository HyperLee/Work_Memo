...
原始題目、需求與參考連結保留於 動態規劃測試/動態規劃測試/Program.cs 的 XML 註解。
...
dotnet restore 動態規劃測試/動態規劃測試/動態規劃測試.csproj
dotnet build 動態規劃測試/動態規劃測試/動態規劃測試.csproj --nologo
dotnet run --project 動態規劃測試/動態規劃測試/動態規劃測試.csproj --no-build --nologo

本機只有 .NET 10 runtime；net8.0 可執行時使用：
DOTNET_ROLL_FORWARD=Major dotnet run --project 動態規劃測試/動態規劃測試/動態規劃測試.csproj --no-build --nologo
...
```text
[封裝-銀行帳戶]
Expected: balance=1200;withdraw=true;rejected=true
Actual: balance=1200;withdraw=true;rejected=true
PASS-FAIL: PASS
[繼承-多型]
Expected: Vehicle|Car:4|Motorcycle:跑車
Actual: Vehicle|Car:4|Motorcycle:跑車
PASS-FAIL: PASS
[介面-付款]
Expected: employee=8000;invoice=1000
Actual: employee=8000;invoice=1000
PASS-FAIL: PASS
[抽象類別-形狀]
Expected: circle=78.539816;rectangle=24
Actual: circle=78.539816;rectangle=24
PASS-FAIL: PASS
