...
原始需求與題目說明保留在 `參考傳回ref用法/參考傳回ref用法/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore 參考傳回ref用法/參考傳回ref用法/參考傳回ref用法.csproj
dotnet build 參考傳回ref用法/參考傳回ref用法/參考傳回ref用法.csproj --nologo
dotnet run --project 參考傳回ref用法/參考傳回ref用法/參考傳回ref用法.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project 參考傳回ref用法/參考傳回ref用法/參考傳回ref用法.csproj --no-build --nologo
```
...
Case: 值傳遞、ref 與 out
Expected: normal=20; return=1234; ref=777; out=888
Actual: normal=20; return=1234; ref=777; out=888
PASS-FAIL: PASS
