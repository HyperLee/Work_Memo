...
原始題目、需求與參考連結保留於 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/Program.cs 的 XML 註解。
...
dotnet restore 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容.csproj
dotnet build 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容.csproj --nologo
dotnet run --project 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容.csproj --no-build --nologo

本機只有 .NET 10 runtime；net8.0 可執行時使用：
DOTNET_ROLL_FORWARD=Major dotnet run --project 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容.csproj --no-build --nologo
...
```text
.
├── 順時針_螺旋_輸出矩陣內容/順時針_螺旋_輸出矩陣內容/
│   ├── Program.cs
│   └── 順時針_螺旋_輸出矩陣內容.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
