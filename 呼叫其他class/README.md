...
原始需求與題目說明保留在 `呼叫其他class/呼叫其他class/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore 呼叫其他class/呼叫其他class/呼叫其他class.csproj
dotnet build 呼叫其他class/呼叫其他class/呼叫其他class.csproj --nologo
dotnet run --project 呼叫其他class/呼叫其他class/呼叫其他class.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project 呼叫其他class/呼叫其他class/呼叫其他class.csproj --no-build --nologo
```
