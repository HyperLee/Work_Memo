...
原始題目、需求與參考連結保留於 BitwiseSubsequence/BitwiseSubsequence/Program.cs 的 XML 註解。
...
dotnet restore BitwiseSubsequence/BitwiseSubsequence/BitwiseSubsequence.csproj
dotnet build BitwiseSubsequence/BitwiseSubsequence/BitwiseSubsequence.csproj --nologo
dotnet run --project BitwiseSubsequence/BitwiseSubsequence/BitwiseSubsequence.csproj --no-build --nologo

本機只有 .NET 10 runtime；net8.0 可執行時使用：
DOTNET_ROLL_FORWARD=Major dotnet run --project BitwiseSubsequence/BitwiseSubsequence/BitwiseSubsequence.csproj --no-build --nologo
...
```text
.
├── BitwiseSubsequence/BitwiseSubsequence/
│   ├── Program.cs
│   └── BitwiseSubsequence.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
