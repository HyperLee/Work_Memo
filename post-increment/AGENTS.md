...
This directory contains the .NET 10 console project `post-increment`.

- `post-increment/post-increment.csproj` is the SDK-style executable project targeting `net10.0`.
- `post-increment/Program.cs` is the preserved console entry point and teaching example.
- `post-increment.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug post-increment` configuration and its matching build task.
