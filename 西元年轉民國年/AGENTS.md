...
This directory contains the .NET 10 console project `西元年轉民國年`.

- `西元年轉民國年/西元年轉民國年.csproj` is the SDK-style executable project targeting `net10.0`.
- `西元年轉民國年/Program.cs` is the preserved console entry point and teaching example.
- `西元年轉民國年.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 西元年轉民國年` configuration and its matching build task.
