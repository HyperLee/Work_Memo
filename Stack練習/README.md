...
原始需求與題目說明保留在 `Stack練習/Stack練習/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore Stack練習/Stack練習/Stack練習.csproj
dotnet build Stack練習/Stack練習/Stack練習.csproj --nologo
dotnet run --project Stack練習/Stack練習/Stack練習.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project Stack練習/Stack練習/Stack練習.csproj --no-build --nologo
```
...
```text
Case: LIFO 操作
Expected: top=5; pop=5; peek=4; count=4; clear=0
Actual: top=5; pop=5; peek=4; count=4; clear=0
PASS-FAIL: PASS

Case: 空堆疊重新建立
Expected: top=9; pop=9; peek=7; count=1; clear=0
Actual: top=9; pop=9; peek=7; count=1; clear=0
PASS-FAIL: PASS
