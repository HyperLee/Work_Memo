...
This directory contains the .NET 10 console project `IList宣告與初始化方法`.

- `IList宣告與初始化方法/IList宣告與初始化方法.csproj` is the SDK-style executable project targeting `net10.0`.
- `IList宣告與初始化方法/Program.cs` is the preserved console entry point and teaching example.
- `IList宣告與初始化方法.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug IList宣告與初始化方法` configuration and its matching build task.
