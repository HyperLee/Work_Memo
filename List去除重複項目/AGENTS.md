...
This directory contains the .NET 10 console project `List去除重複項目`.

- `List去除重複項目/List去除重複項目.csproj` is the SDK-style executable project targeting `net10.0`.
- `List去除重複項目/Program.cs` is the preserved console entry point and teaching example.
- `List去除重複項目.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug List去除重複項目` configuration and its matching build task.
