...
This directory contains the .NET 10 console project `Json_reader`.

- `Json_reader/Json_reader.csproj` is the SDK-style executable project targeting `net10.0`.
- `Json_reader/Program.cs` is the preserved console entry point and teaching example.
- `Json_reader.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug Json_reader` configuration and its matching build task.
