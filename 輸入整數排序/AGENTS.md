...
This directory contains the .NET 10 console project `輸入整數排序`.

- `輸入整數排序/輸入整數排序.csproj` is the SDK-style executable project targeting `net10.0`.
- `輸入整數排序/Program.cs` is the preserved console entry point and teaching example.
- `輸入整數排序.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 輸入整數排序` configuration and its matching build task.
