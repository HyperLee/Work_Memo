...
This directory contains the .NET 10 console project `優化遞迴方式`.

- `優化遞迴方式/優化遞迴方式.csproj` is the SDK-style executable project targeting `net10.0`.
- `優化遞迴方式/Program.cs` is the preserved console entry point and teaching example.
- `優化遞迴方式.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 優化遞迴方式` configuration and its matching build task.
