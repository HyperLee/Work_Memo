...
This directory contains the .NET 10 console project `C# OOP 繼承`.

- `C# OOP 繼承/C# OOP 繼承.csproj` is the SDK-style executable project targeting `net10.0`.
- `C# OOP 繼承/Program.cs` is the preserved console entry point and teaching example.
- `C# OOP 繼承.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug C# OOP 繼承` configuration and its matching build task.
