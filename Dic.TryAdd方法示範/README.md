...
原始需求與題目說明保留在 `Dic.TryAdd方法示範/Dic.TryAdd方法示範/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore Dic.TryAdd方法示範/Dic.TryAdd方法示範/Dic.TryAdd方法示範.csproj
dotnet build Dic.TryAdd方法示範/Dic.TryAdd方法示範/Dic.TryAdd方法示範.csproj --nologo
dotnet run --project Dic.TryAdd方法示範/Dic.TryAdd方法示範/Dic.TryAdd方法示範.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project Dic.TryAdd方法示範/Dic.TryAdd方法示範/Dic.TryAdd方法示範.csproj --no-build --nologo
```
...
```text
.
├── Dic.TryAdd方法示範/Dic.TryAdd方法示範/
│   ├── Program.cs
│   └── Dic.TryAdd方法示範.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
