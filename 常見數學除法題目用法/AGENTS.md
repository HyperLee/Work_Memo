...
This directory contains the .NET 10 console project `常見數學除法題目用法`.

- `常見數學除法題目用法/常見數學除法題目用法.csproj` is the SDK-style executable project targeting `net10.0`.
- `常見數學除法題目用法/Program.cs` is the preserved console entry point and teaching example.
- `常見數學除法題目用法.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 常見數學除法題目用法` configuration and its matching build task.
