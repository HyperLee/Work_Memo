...
This directory contains the .NET 10 console project `隱碼-保留前六個字`.

- `隱碼-保留前六個字/隱碼-保留前六個字.csproj` is the SDK-style executable project targeting `net10.0`.
- `隱碼-保留前六個字/Program.cs` is the preserved console entry point and teaching example.
- `隱碼-保留前六個字.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 隱碼-保留前六個字` configuration and its matching build task.
