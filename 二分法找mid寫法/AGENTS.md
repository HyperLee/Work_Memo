...
This directory contains the .NET 10 console project `二分法找mid寫法`.

- `二分法找mid寫法/二分法找mid寫法.csproj` is the SDK-style executable project targeting `net10.0`.
- `二分法找mid寫法/Program.cs` is the preserved console entry point and teaching example.
- `二分法找mid寫法.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug 二分法找mid寫法` configuration and its matching build task.
