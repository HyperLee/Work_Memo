...
This directory contains the .NET 10 console project `Fibonacci`.

- `Fibonacci/Fibonacci.csproj` is the SDK-style executable project targeting `net10.0`.
- `Fibonacci/Program.cs` is the preserved console entry point and teaching example.
- `Fibonacci.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug Fibonacci` configuration and its matching build task.
