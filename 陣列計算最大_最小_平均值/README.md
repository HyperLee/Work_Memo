...
原始題目、需求與參考連結保留於 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/Program.cs 的 XML 註解。
...
dotnet restore 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/陣列計算最大_最小_平均值.csproj
dotnet build 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/陣列計算最大_最小_平均值.csproj --nologo
dotnet run --project 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/陣列計算最大_最小_平均值.csproj --no-build --nologo

本機只有 .NET 10 runtime；net8.0 可執行時使用：
DOTNET_ROLL_FORWARD=Major dotnet run --project 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/陣列計算最大_最小_平均值.csproj --no-build --nologo
...
```text
Case: 題目範例 / LINQ
Expected: max=90; min=10; average=48.625
Actual: max=90; min=10; average=48.625
PASS-FAIL: PASS

Case: 題目範例 / 迴圈
Expected: max=90; min=10; average=48.63
Actual: max=90; min=10; average=48.63
PASS-FAIL: PASS

Case: 包含負數 / LINQ
Expected: max=5; min=-5; average=0
Actual: max=5; min=-5; average=0
PASS-FAIL: PASS

Case: 包含負數 / 迴圈
Expected: max=5; min=-5; average=0.00
Actual: max=5; min=-5; average=0.00
PASS-FAIL: PASS
...
```text
.
├── 陣列計算最大_最小_平均值/陣列計算最大_最小_平均值/
│   ├── Program.cs
│   └── 陣列計算最大_最小_平均值.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
