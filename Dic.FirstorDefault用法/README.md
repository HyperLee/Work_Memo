# Dic.FirstorDefault用法

這個 net8.0 主控台專案保留原始教學程式與公開方法，並以固定 smoke test 驗證可重現結果。

## 題目或原始需求說明

原始需求與題目說明保留在 `Dic.FirstorDefault用法/Dic.FirstorDefault用法/Program.cs` 的 XML 註解中。本 README 補充執行契約與翻新後的驗收方式。
...
```bash
dotnet restore Dic.FirstorDefault用法/Dic.FirstorDefault用法/Dic.FirstorDefault用法.csproj
dotnet build Dic.FirstorDefault用法/Dic.FirstorDefault用法/Dic.FirstorDefault用法.csproj --nologo
dotnet run --project Dic.FirstorDefault用法/Dic.FirstorDefault用法/Dic.FirstorDefault用法.csproj --no-build --nologo
```

目前主機若只有 .NET 10 runtime，執行 net8/net9 成品時可使用：

```bash
DOTNET_ROLL_FORWARD=Major dotnet run --project Dic.FirstorDefault用法/Dic.FirstorDefault用法/Dic.FirstorDefault用法.csproj --no-build --nologo
```
...
Case: 負數也可使用 XOR / 方法 1
Expected: -3
Actual: -3
PASS-FAIL: PASS

Case: 負數也可使用 XOR / 方法 2
Expected: -3
Actual: -3
PASS-FAIL: PASS

Case: 負數也可使用 XOR / 方法 3
Expected: -3
Actual: -3
PASS-FAIL: PASS
...
```text
.
├── Dic.FirstorDefault用法/Dic.FirstorDefault用法/
│   ├── Program.cs
│   └── Dic.FirstorDefault用法.csproj
├── .editorconfig
├── .gitattributes
├── .gitignore
├── AGENTS.md
