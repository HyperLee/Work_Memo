...
This directory contains the .NET 10 console project `OOP-class-1`.

- `OOP-class-1/OOP-class-1.csproj` is the SDK-style executable project targeting `net10.0`.
- `OOP-class-1/Program.cs` is the preserved console entry point and teaching example.
- `OOP-class-1.sln` remains the solution entry point for solution-level builds.
...
Restore resolves dependencies, build compiles the solution, and run executes the console smoke test. In VS Code, F5 uses the `Debug OOP-class-1` configuration and its matching build task.
